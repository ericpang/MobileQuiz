...
var quizMaster = (function () {
	var name;				// q1.json
	var data;				// data of json file
	var loaded = false;
	var displayDom;			// the DOM where the question appears
	var successCbAlias;		// callback
	...
	function displayQuiz(successCb) {
		...
		if(current.state === "introduction") {
			...
			html = "<h2>" + current.question.question + 
					"</h2><form><div data-role='fieldcontain'><fieldset data-role='controlgroup'>";
			
			if ( current.question.picture != null ) {
				html += "<img src='img/"+current.question.picture+"' alt='' />";
			}
			...
			html = "<h2>Complete!</h2><br/><h3>The "+ name +" is now complete. You got "+current.correct+" correct out of "+data.questions.length+".</h3>";
			html += "<a href='index.html' data-role='button'>Next Station</a>";
